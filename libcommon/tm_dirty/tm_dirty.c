...
#include <libcommon/inet/inet_utils.h>
...
int tm_dirty_load_from_server(void)
{
    ...
    for (i = 0; i < tm_dirty_valid_server_count; i++){
        ...
        if ((fd = safe_tcp_connect(tm_dirty_server_ip[flag], tm_dirty_server_port[flag], 2, 0)) != -1){
			...
			DEBUG_LOG("---Frankie---'%s' %u", tm_dirty_server_ip[flag], tm_dirty_server_port[flag]);
		}
    }
    if (fd == -1) {
        if (tm_dirty_warning_func != NULL) {
            char ip[16];
...
recvok:			
		...
		if (fp == NULL) {
			...
			if (fp == NULL) {
				close(fd);
				ERROR_RETURN(("tm_dirty: can't create new dirty file"), -1);
			}
			file_closed = 0;
		}
...
int tm_load_dirty(const char *conf_file_path)
{
	if (daemon_called)	{//如果已经调用了daemon函数 则此接口无效
		...
		BOOT_LOG(-1, "reason 1: needed to update asyncserver to 1.6.1;\nreason2: old interface [tm_load_dirty] called, but variable 'tm_dirty_use_dirty_logical is set to 0'");
		return -1;
	}
}
...
int tm_dirty_check(int level, char *msg)
{
	...
	return acsmSearch(tm_dirty_handle[flag], (unsigned char *)msg, strlen(msg));
}
...
int tm_dirty_replace(char *msg)
{
	...
	return acsm_pattern_replace(tm_dirty_handle[flag], (unsigned char *)msg, strlen(msg));
}
...
int tm_dirty_check_where(int level, char *msg, char *retbuf)
{
	...
	return acsm_get_first_dirty_word(tm_dirty_handle[flag], (unsigned char *)msg, strlen(msg), (unsigned char *)retbuf);
...
int tm_dirty_list_word(int char_set)
{
	...
	if (tm_dirty_handle[flag] != NULL) {
		...
		if (count == 0)
			printf("no dirty words found but dirty_handle is exist\n");
	} else {
		printf ("no dirty words found and dirty_handle is [NOT] exist!!\n");
	}
	return 0;
}
