...
*   Search Text or Binary Data for Pattern matches
*/
int acsm_get_first_dirty_word(ACSM_STRUCT * acsm, unsigned char *Tx, int n, unsigned char *retbuf)
{
	int state;
	unsigned char Tc[MAX_STR_BUFFER];//1M,even use utf8 with 4bytes one symbol, this buffer can hold 250K symbol
	//ACSM_PATTERN * mlist;
...
* @return 0:干净串,1:替换过.
*/
int acsm_pattern_replace(ACSM_STRUCT * acsm, unsigned char *Tx, int n)
{
	int state;
