...
#define	MAX_PATTERNS		(8192)		//最大8192个脏词
#define	MAX_ADS_LEN			(4096)		//广告长度
#define	MAX_ADS				(2048)		//最大2048条广告
#define	MAX_STATE			(256 * 1024) //最多20W个节点 256*1k*1k＝256M
...
*   Prototypes
*/
extern ACSM_STRUCT * acsmNew();

extern void acsmNew_with_fd(int fd, ACSM_STRUCT **handle1, ACSM_STRUCT **handle2);

extern int acsmAddPattern( ACSM_STRUCT * p, unsigned char * pat, int n);
extern int acsmCompile ( ACSM_STRUCT * acsm );
extern int acsmSearch (ACSM_STRUCT * acsm, unsigned char *Tx, int n); //返回查找到的个数
extern void acsmFree ( ACSM_STRUCT * acsm );
extern int acsm_get_first_dirty_word(ACSM_STRUCT * acsm, unsigned char *Tx, int n, unsigned char *retbuf);

extern int acsm_pattern_replace(ACSM_STRUCT * acsm, unsigned char *Tx, int n);
