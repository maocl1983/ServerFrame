...
void write_log_for_thread(int lvl, uint32_t key, const char *buffer, int len)
{
	if (lvl > log_level) {
		return;
	}
	...
	if (unlikely(len >= log_buf_sz - pos)) {
		len = log_buf_sz - pos;
	}
	memcpy(log_buffer + pos, buffer, len);
	log_buffer[log_buf_sz - 1] = '\n';

	write(fds_info[lvl].opfd, log_buffer, pos + len);
}
